    ...
    private inner class ScaleListener : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            if (origWidth * saveScale <= width || origHeight * saveScale <= height) {
                matrix_.postScale(mScaleFactor, mScaleFactor, width / 2, height / 2)
                if (mScaleFactor < 1) {
                    matrix_.getValues(m)
                    ...
                    if (mScaleFactor < 1) {
                        if (Math.round(origWidth * saveScale) < width) {
                            if (y < -bottom) matrix_.postTranslate(
                                0f,
                                -(y + bottom)
                            ) else if (y > 0) matrix_.postTranslate(0f, -y)
                        } else {
                            if (x < -right) matrix_.postTranslate(
                                -(x + right),
                                0f
                            ) else if (x > 0) matrix_.postTranslate(-x, 0f)
                        }
                    }
                }
            } else {
